...
class FakeTaskTest(absltest.TestCase):
  ...
  def _assert_compare_to_fake_dataset(
      self, ds, split, features, token_preprocessed=False):
      ...
      for n, ex in enumerate(fake_examples):
        if feat.add_eos:
          fake_examples[n][key] = ex[key][
              :self._SEQUENCE_LENGTH[key] - 1] + (1,)
        else:
          fake_examples[n][key] = ex[key][:self._SEQUENCE_LENGTH[key]]
  ...
  def verify_task_matches_fake_datasets(
    ...
    for split in splits:
      ...
      if num_shards:
        ...
        ds = get_dataset()
      self._assert_compare_to_fake_dataset(
          ds,
          split,
